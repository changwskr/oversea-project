...
import com.skcc.oversea.user.domain.UserActivity;
import com.skcc.oversea.user.infrastructure.jpa.UserActivityEntity;
import com.skcc.oversea.user.infrastructure.jpa.UserActivityRepositoryJpa;
import com.skcc.oversea.user.service.port.UserActivityRepositoryPort;
...
public class UserActivityRepositoryPortJpaImpl implements UserActivityRepositoryPort {
    ...
    public UserActivity save(UserActivity userActivity) {
        log.info("[UserActivityRepositoryPortJpaImpl] save START - userId: {}, activityType: {}", userActivity.getUserId(), userActivity.getActivityType());
    ...
    public Page<UserActivity> findByUserId(Long userId, Pageable pageable) {
        log.info("[UserActivityRepositoryPortJpaImpl] findByUserId(Pageable) START - userId: {}, pageable: {}", userId, pageable);
        
        Page<UserActivity> result = userActivityRepositoryJpa.findByUserIdOrderByActivityTimeDesc(userId, pageable)
                .map(UserActivityEntity::toDomain);
        
        log.info("[UserActivityRepositoryPortJpaImpl] findByUserId(Pageable) END - totalElements: {}", result.getTotalElements());
        return result;
    }
    ...
    public List<UserActivity> findByActivityTimeBetween(LocalDateTime startDate, LocalDateTime endDate) {
        log.info("[UserActivityRepositoryPortJpaImpl] findByActivityTimeBetween START - startDate: {}, endDate: {}", startDate, endDate);
    ...
    @Override
    public List<UserActivity> findByUserIdAndActivityTimeBetween(Long userId, LocalDateTime startDate, LocalDateTime endDate) {
        log.info("[UserActivityRepositoryPortJpaImpl] findByUserIdAndActivityTimeBetween START - userId: {}, startDate: {}, endDate: {}", userId, startDate, endDate);
        
        List<UserActivity> result = userActivityRepositoryJpa.findByUserIdAndActivityTimeBetween(userId, startDate, endDate)
                .stream()
                .map(UserActivityEntity::toDomain)
                .collect(Collectors.toList());
        
        log.info("[UserActivityRepositoryPortJpaImpl] findByUserIdAndActivityTimeBetween END - count: {}", result.size());
        return result;
    }
