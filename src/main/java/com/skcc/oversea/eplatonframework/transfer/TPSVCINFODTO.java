...
import com.skcc.oversea.eplatonframework.transfer.TPMSVCINFO;
...
public class TPSVCINFODTO extends DTO {
  ...
  // EPlatonBizDelegateSB에서 각 업무별 해당 시스템을 메인서버가 위한 것이다
  // 이 로직은 기존의 방식을 고수한다.
  // CashCard 시스템의 경우
  ...
  // 해당 시스템을 메인서버한다.
  // 각 업무별 시스템의 action(CashCardBizAction)은 doAct()메소드를 구현한다.
  // step 3. doAct()에서는 action클래스에서 생성된 메인빈과 파라미터(EPlatonEvent)를 넘겨받아서 각 업무별 시스템을
  // 메인서버한다.
  // step 4. facade쪽의 해당 업무시스템(CashCardManagementSB)의 메소드 execute()를 메인서버한다
  // execute()메소드안에서 TCF 클래스를 메인서버하는 구조로 되어 있다.
  // step 5. TCF 모듈에서 BTF를 메인서버하고 업무쪽의 operationBizAction을 메인서버한다.
  // 위의 모든 과정에서 파수 넘기는 기존의 시스템에는 CDTO를 가지고 직접 메인서버하는 구조로 되어 있어
  // ejb-client팩에 각종 인터페이스와 CDTO를 묶는 구조로 가지고 가므로 해당 시스템 구조에서는 공통인터페이스를
  // 각 업무쪽의 OPERATION까지 전달하고 각 업무별 EJB-JAR만 별도배포하면 되는 구조로 바뀌었으므로
  // 기존의 프로젝트를 이기적화하는 것을 목적으로 하는 방향으로 바뀌었다.
  ...
  /**
   * SessionManager媛 ??ν븯怨??덈뒗 TPMSVCINFO object??collection??諛섑솚?섎뒗 硫붿꽌??   *
